 ...
 * Removes DOM nodes, functions, and circular references
 */
function sanitizeForJSON(obj, seen = new WeakSet()) {
  ...
  // Skip React internals and other non-serializable objects
  if (obj.$$typeof || obj._reactInternals || obj._reactFiber) {
    return undefined;
  }
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    // Skip 'ref' properties that might hold DOM references
    if (key === 'ref' || key === 'current') {
      if (value && typeof value === 'object' &&
          (value instanceof HTMLElement || value instanceof Node)) {
        continue;
      }
    }
...
export function useTemplateStorage({
  ...
  const buildTemplateData = useCallback((name = templateName) => ({
    ...
    version: '2.0',
    data: {
      elements: sanitizeForJSON(elements),
      sections: sanitizeForJSON(sections),
      canvasSettings: {
        width: '210mm',
        height: '297mm',
   ...
   * FIXED: Now sanitizes data to prevent circular reference errors
   */
